...
public class Aerolinea
{
    ...
    public Vuelo getVuelo( String codigoRuta, String fechaVuelo )
    {
        for (Vuelo v : vuelos) {
            if (v.getRuta() != null
                    && v.getRuta().getCodigoRuta().equals(codigoRuta)
    ...
    {
    	Collection<Tiquete> t = new ArrayList<Tiquete>();
        for(Vuelo v : vuelos) {
        	Collection<Tiquete> tEx = v.getTiquetes();
        	t.addAll(tEx);
        }
        return t;
    ...
    public void programarVuelo( String fecha, String codigoRuta, String nombreAvion ) throws Exception
    {
        ...
        if (ruta == null) throw new Exception("Ruta inexistente: " + codigoRuta);

        Avion avion = null;
        for (Avion a : aviones) {
            if (a.getNombre().equals(nombreAvion)) { avion = a; break; }
        }
        if (avion == null) throw new Exception("Avión inexistente: " + nombreAvion);

        for (Vuelo v : vuelos) {
            if (v.getAvion().equals(avion) && v.getFecha().equals(fecha)) {
                int s1 = Ruta.getHoras(ruta.getHoraSalida())*60 + Ruta.getMinutos(ruta.getHoraSalida());
                int e1 = Ruta.getHoras(ruta.getHoraLlegada())*60 + Ruta.getMinutos(ruta.getHoraLlegada());
                int s2 = Ruta.getHoras(v.getRuta().getHoraSalida())*60 + Ruta.getMinutos(v.getRuta().getHoraSalida());
                int e2 = Ruta.getHoras(v.getRuta().getHoraLlegada())*60 + Ruta.getMinutos(v.getRuta().getHoraLlegada());
                if (e1 <= s1) e1 += 24*60;
                if (e2 <= s2) e2 += 24*60;
                if (s1 < e2 && s2 < e1) throw new Exception("El avión ya está ocupado en esa franja");
            }
        }

        vuelos.add(new Vuelo(avion, fecha, ruta));
    }
    ...
    public int venderTiquetes( String identificadorCliente, String fecha, String codigoRuta, int cantidad ) throws VueloSobrevendidoException, Exception
    {
        ...
        if (cliente == null) throw new Exception("Cliente inexistente: " + identificadorCliente);

        Vuelo vuelo = getVuelo(codigoRuta, fecha);
        if (vuelo == null) throw new Exception("Vuelo inexistente para la ruta y fecha dadas");

        int mes = 1;
        try {
            if (fecha.contains("-")) { String[] p = fecha.split("-"); mes = Integer.parseInt(p[1]); }
            else if (fecha.contains("/")) { String[] p = fecha.split("/"); mes = Integer.parseInt(p[1]); }
        } catch (Exception e) { mes = 1; }

        CalculadoraTarifas calc =
            ((mes>=1 && mes<=5) || (mes>=9 && mes<=11))
