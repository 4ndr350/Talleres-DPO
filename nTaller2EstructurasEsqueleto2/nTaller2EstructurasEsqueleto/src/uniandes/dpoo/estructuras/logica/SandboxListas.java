...
public class SandboxListas
{
    ...
    {
    	listaCadenas.removeIf(v -> v.equals(cadena));
    ...
    {
    	if(posicion < 0) listaEnteros.addFirst(entero);
    	else if (posicion >= listaEnteros.size()) agregarEntero(entero); 
    ...
    public void reiniciarArregloEnteros( double[] valores )
    {
    	...
    	for (int i = 0; i < valores.length; i++) {
    		int act = (int) valores[i];
    		listaEnteros.add(act);
    	}
    }
    ...
    public int contarApariciones( String cadena )
    {
    	...
    	for (String i : listaCadenas) {
    		if (i.equalsIgnoreCase(cadena)) num++;
    	}
        return num;
    }
